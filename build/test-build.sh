...
if node build/verify-invariants.js test-fixtures/invariants/stroke-only-rawhex.json --pre-bind >/dev/null 2>&1; then
...
if node build/verify-invariants.js test-fixtures/invariants/stroke-only-bound-clean.json --pre-bind >/dev/null 2>&1; then
